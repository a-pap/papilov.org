---
title: "彭罗斯图"
date: 2026-03-22
lastmod: 2026-03-22
version: "1.0"
status: "finished"
confidence: "certain"
description: "如何将无限的时空放在一张有限的纸上——以及为什么这很重要。共形图的视觉指南：闵可夫斯基、史瓦西、德西特、反德西特。"
tags: ["physics", "general-relativity", "penrose-diagrams", "black-holes", "cosmology"]
series: "physics-explainers"
ai_disclosure: "研究由Claude（Anthropic）辅助完成。编辑方向与领域专业知识由作者提供。"
license: "CC BY-NC 4.0"
---

时空是无限的。光可以永远传播。黑洞将一个点压缩到零。如何在一张有限的纸上画出这一切？

罗杰·彭罗斯在1960年代初给出的答案是：共形紧化。一种数学变换，将无穷远压缩为有限的点和线，同时保留因果物理中唯一重要的东西——光锥结构。

<div style="margin: 2rem 0; padding: 1.5rem; border: 1px solid var(--border-color); border-radius: 8px; background: var(--bg-secondary);">

**[打开交互式视觉指南 →](/research/penrose-diagrams/infographic.html)**

6个部分，包含注释SVG图表。4种时空的交互式图集：闵可夫斯基、史瓦西、德西特、反德西特。

</div>

## 所有图的五条规则

时间向上流动。空间水平延伸。光始终以45°行进。有质量的物体沿更陡的路径运动。每个点代表一个2-球面。

## 四种基本时空

平坦的闵可夫斯基空间呈菱形。史瓦西黑洞在顶部添加锯齿形奇点和45°的事件视界。德西特空间（大致对应我们的宇宙）呈正方形。反德西特空间呈垂直条带，其类时边界上住着对偶的共形场论——这正是AdS/CFT对应的基础。

## 参考文献

- R. Penrose, *Phys. Rev. Lett.* **10**, 66 (1963)
- R. Penrose, *Phys. Rev. Lett.* **14**, 57 (1965) — 2020年诺贝尔物理学奖
- S. W. Hawking, G. F. R. Ellis, *The Large Scale Structure of Space-Time*, Cambridge (1973)
